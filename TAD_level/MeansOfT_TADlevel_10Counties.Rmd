---
title: "MeansOfT_TADlevel_10Counties"
---

# Summary

* Process the CTPP data: Minority Status at the TAD level for 10 counties (9 DVRPC counties + New Castle, DE.) 

* Input: CVS Data extracted from the CTPP site 

* Output: Produces a feature class at the TAD level with two data colums: (1) where do residents of philadelphia work? and (2) where do workers in philadelphia come from? Both are merged and exported into the same Shp. The same data is also exported to Excel.
...
tableName <- "Minority_TADlevel_10Counties"
...
ggplot() + geom_sf(data = GeoTenCountyTotals,  aes(fill = RESIDENCE)) + theme_bw() + ditch_the_axes + scale_fill_gradient(low="#ccccff", high="#0000e6")

ggplot() + geom_sf(data = GeoTenCountyTotals,  aes(fill = WORKPLACE)) + theme_bw() + ditch_the_axes + scale_fill_gradient(low="#ccccff", high="#0000e6")
...
shpToWrite <- paste(home, outFolder, "TenCountyTotals.shp", sep="")
#shpToWrite
...
fileToWrite <- paste(home, outFolder,"TenCountyTotals.xlsx", sep="")
