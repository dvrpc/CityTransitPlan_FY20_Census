---
title: "Minority_Status_TADlevel_10Counties_ImprovedAccuracy"
---

# Summary

* Process the CTPP data: Minority Status at the TAD level for 10 counties (9 DVRPC counties + New Castle, DE.). The goal is to get numbers that are more accurate than the original TAD ones by using other data sources
...
aggregatetoTADs <- function(ctppData, residence){
  ...
  if (residence == "PHL"){
    ...
    ctppDataWithTADs <- ctppDataWithTADs[c("WORKPLACE", "neutral_id",  "Total", 
                                          "White_not_hisp", "Minority" )]
  }else{
    ctppDataWithTADs <- merge(ctppData, munToTADs, by.x="RESIDENCE", by.y="name")
    ctppDataWithTADs <- ctppDataWithTADs[c("RESIDENCE", "neutral_id",  "Total", 
                                          "White_not_hisp", "Minority" )]
  }
...
MultiplyByFactor <- function(ctppData, rectifyingfactor){
  ctppData[] = lapply(ctppData, FUN = function(x) if (is.numeric(x)) 
    return(x * rectifyingfactor) else return(x))
  ctppData
}  
...
PANJ_suburbs <- mergeBothFlows(PHLToSuburbs, SuburbsToPHL)
NewCastle <- mergeBothFlows(PHLToNewCastleRectified, NewCastleToPHLRectified)
Philadelphia <- mergeBothFlows(PHLToPHLResRectified, PHLToPHLWpRectified)
...
fileToWrite <- paste(home, outFolder,"MinorityStatus_TADLevel_10Counties.xlsx", sep="")
